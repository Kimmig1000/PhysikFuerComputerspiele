...
public class WurfParabel implements WindowListener, GLEventListener, KeyListener {
	...
	// meine Alte
	/*
	 * public void zeichneKreis(GL3 gl, float x1, float y1) {
	 * mygl.rewindBuffer(gl); // Vertex-Buffer zuruecksetzen
	 * 
	 * double dt = 2.0*Math.PI / 40; for (int i=0; i<40; i++){ float x1double =
	 * (float) (x1*Math.cos(i*dt)); float x2double = (float)
	 * (y1*Math.sin(i*dt)); mygl.putVertex(x1double,x2double, 0f); // Eckpunkte
	 * in VertexArray speichern
	 * 
	 * }
	 */
